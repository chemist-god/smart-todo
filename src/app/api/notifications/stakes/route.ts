        ...
        for (const stake of stakes) {
            ...
            if (stake.status === 'COMPLETED') {
                ...
                notifications.push({
                    ...
                    message: `Congratulations! You completed "${stake.title}" and earned Gh${totalReward.toFixed(2)}`,
            ...
            if (stake.status === 'FAILED') {
                notifications.push({
                    ...
                    type: 'penalty',
                    title: 'Stake Failed',
                    message: `"${stake.title}" was not completed on time. Penalty: Gh${Number(stake.userStake).toFixed(2)}`,
                    timestamp: now.toISOString(),
                    read: false,
                    stakeId: stake.id,
            ...
            if (stake.stakeType === 'SOCIAL_STAKE' && stake.participants.length > 0) {
                ...
                for (const participant of recentParticipants) {
                    notifications.push({
                        ...
                        message: `${participant.participantName} joined your stake "${stake.title}" with Gh${Number(participant.amount).toFixed(2)}`,
